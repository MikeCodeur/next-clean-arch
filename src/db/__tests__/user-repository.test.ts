...
describe('Operations for User', () => {
  beforeAll(async () => {
    await initDrizzle() // Assure-toi que la base de données est propre
  })

  test.sequential('should create a new user', async () => {
  ...
  test.sequential('should read all users', async () => {
    const users = await getUsersDao()
    expect(users.length).toBeGreaterThan(0)
  })

  test.sequential('should read a user by ID', async () => {
    const users = await getUsersDao()
  ...
  test.sequential('should get user by email', async () => {
    const users = await getUsersDao()
    const firstUser = users[0]
  ...
  test.sequential('should create user , account and session', async () => {
    const newUser: CreateUser = {
      name: faker.person.firstName(),
      email: faker.internet.email(),
      password: faker.internet.password(),
  ...
  test.sequential(
    'should throw error and not create user with transaction',
    async () => {
      const newUser: CreateUser = {
        name: 'John Doe',
        email: faker.internet.email(),
