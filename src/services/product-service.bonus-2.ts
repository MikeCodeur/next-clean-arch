...
export async function createProductWithCategoryService(
  ...
  logger.info(`createProductWithCategoryService : params`, {
    productName,
    categoryName,
  })
...
export async function persistProductService(product: CreateEditProduct) {
  logger.info(`persistProductService`)
...
export const deleteProductService = async (productParams: DeleteProduct) => {
  logger.info(`deleteProductService `, {
    productParams,
  })
  const canCreate = await canMutateProduct()
  if (!canCreate) {
    logger.error(
      `deleteProductService : "Vous n'êtes pas autorisé à supprimer un produit"`
    )
    throw new AuthorizationError(
      "Vous n'êtes pas autorisé à supprimer un produit"
    )
  }
...
export async function getProductsPaginationService(
  ...
  logger.info(`getProductsPaginationService : params `, {
    nbElement,
    start,
  })
